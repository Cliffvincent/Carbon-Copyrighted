...
class Editor extends React.Component {
  ...
  async componentDidMount() {
    ...
    (async () => {
      ...
      if (file) {
        const { content, error } = await replit.readFile(file);
        this.setState({ code: content, usedFileHandler: true })
      }
    })();
  ...
  exportImage = (format = 'blob', options = {}) => {
    ...
    const downloadToRepl = options.downloadToRepl || false
    ...
    if (downloadToRepl) {
      return this.getCarbonImage({ format })
        ...
        .then(buf => {
          replit.fs.writeFile(`${prefix}.${format === 'svg' ? 'svg' : 'png'}`, buf)
        })
    } else {
      return this.getCarbonImage({ format })
