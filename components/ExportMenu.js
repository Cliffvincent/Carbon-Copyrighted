...
function ExportMenu({ onChange, exportSize, isVisible, toggleVisibility, exportImage: exp }) {
  ...
  const handleExport = (format, opts) => () =>
    exportImage(format, { ...opts, filename: input.current && input.current.value })
      ...
      >
        ...
        <div className="export-row">
          <Button
            ...
            color={COLORS.PURPLE}
            onClick={() =>
              exportImage('blob', { filename: input.current && input.current.value, open: true, downloadToRepl: true })
            }
          >
            Download to Repl
          </Button>
